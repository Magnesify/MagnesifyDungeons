...
public class Stats implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (commandSender instanceof Player) {
            ...
            dungeonPlayer.messageManager().chat(new LanguageFile().getLanguage().getString("messages.stats.title"));
            ...
            for (int i = 0; i < Math.min(get().getConfig().getInt("settings.top-list-amount"), sortedPlayers.size()); i++) {
                ...
                dungeonPlayer.messageManager().chat(new LanguageFile().getLanguage().getString("messages.stats.sublines").replace("#rank", String.valueOf((i + 1))).replace("#name", player.getName()));
            }
        }
        return false;
    }
